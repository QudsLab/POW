...
# Manually list all C and C++ files (for Windows compatibility)
C_SRCS = \
...
# Map source files to object files in obj\ directory (explicit list for Windows)
# Files stored flat in obj/ with path encoded in filename (e.g., obj\cb_blake3_blake3.obj)
C_OBJS = \
...
obj\mb_argon_thread.obj: src\mb_argon\thread.c | $(OBJ_DIR)
	$(CC) $(CFLAGS) $(INCLUDES) -c $< -o $@
