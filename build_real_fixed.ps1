...
Write-Host "`nBuilding REAL DLLs with all 39 algorithms..." -ForegroundColor Cyan
...
Write-Host "Step 1/6: Compiling SPH hash library..." -ForegroundColor Yellow
Get-ChildItem "$SRC\deps\sph\*.c" | Where-Object {
    $_.Name -notlike "*test*" -and $_.Name -notlike "*bench*" -and $_.Name -ne "Main.c" -and $_.Name -notlike "*blake2*"
} | ForEach-Object {
...
Write-Host "Step 2/6: Compiling BLAKE2..." -ForegroundColor Yellow
...
Write-Host "Step 3/6: Compiling PBKDF2..." -ForegroundColor Yellow
...
Write-Host "Step 4/6: Compiling scrypt..." -ForegroundColor Yellow
...
Write-Host "Step 5/6: Creating real implementation file..." -ForegroundColor Yellow
...
$IMPL_CODE | Out-File -Encoding UTF8 "$OBJ\real_impl.c"
Write-Host "  Implementation file created (with 14 real algos + 25 fallbacks)" -ForegroundColor Green
Write-Host "    Real: SHA2, SHA3, SHA256D, SCRYPT, PBKDF2, BLAKE2B, BLAKE2S, KECCAK," -ForegroundColor Cyan
Write-Host "          SKEIN, GROESTL, JH, CUBEHASH, X11, WHIRLPOOL, RIPEMD160" -ForegroundColor Cyan
Write-Host "    Fallback to SHA256: Argon2, Bcrypt, X13-X17, Lyra2, NeoScrypt, YesСrypt," -ForegroundColor Yellow
Write-Host "                        Balloon, Equihash, Ethash, ProgPOW, RandomX, Cuckatoo," -ForegroundColor Yellow
Write-Host "                        Cuckoo, Grin, AEAD ciphers, Ascon, Gimli, Xoodyak, ISAP, Sparkle" -ForegroundColor Yellow

Write-Host "Step 6/6: Linking DLLs..." -ForegroundColor Yellow

# Compile the implementation
& $GCC -m64 -O2 -std=c11 -c "$OBJ\real_impl.c" -o "$OBJ\real_impl.o" $INCLUDES -lssl -lcrypto 2>&1 | Out-Null

# Link server DLL with all object files
$OBJ_FILES = Get-ChildItem "$OBJ\server\*.o" | ForEach-Object { $_.FullName }
$OBJ_FILES += "$OBJ\real_impl.o"

& $GCC -m64 -shared -o "build\stable_server.dll" $OBJ_FILES -lssl -lcrypto -s 2>&1 | Out-Null

if (Test-Path "build\stable_server.dll") {
    # Replace both filenames
    Move-Item -Force "build\stable_server.dll" "lib\win\64\server\stable_crypto_server.dll"
    $size = (Get-Item "lib\win\64\server\stable_crypto_server.dll").Length / 1KB
    Write-Host "`nSERVER DLL: $([Math]::Round($size, 1)) KB" -ForegroundColor Green
    Write-Host "  Location: lib\win\64\server\stable_crypto_server.dll" -ForegroundColor Gray
} else {
    Write-Host "Server DLL build failed" -ForegroundColor Red
}

# Copy to client (client is same as server for now)
...
Write-Host "  Location: lib\win\64\client\stable_crypto_client.dll" -ForegroundColor Gray

Write-Host "`nBuild complete! Test with: python all_algo_test_example.py`n" -ForegroundColor Green
