...
<#
.SYNOPSIS
    Build script for Windows 32-bit crypto library (server and client)

.DESCRIPTION
    Compiles both server and client DLLs for 32-bit Windows and organizes
    them into lib/win/32/server and lib/win/32/client directories.
    
    Requirements:
    - MSYS2 with MinGW-w64 (mingw32)
    - GCC compiler
    - OpenSSL development libraries
    
.EXAMPLE
    .\build_windows_32.ps1
#>
...
$OUTPUT_DIR = Join-Path $SCRIPT_DIR "lib\win\32"
...
# Compiler flags (32-bit specific)
$COMMON_FLAGS = @(
    "-m32",
    "-O3",
    "-Wall",
...
function Test-Prerequisites {
    ...
    if (-not (Test-Path $MINGW_PATH)) {
        Write-Error "MinGW-w64 (32-bit) not found at $MINGW_PATH"
        Write-Host "Please install MSYS2 from https://www.msys2.org/"
        Write-Host "Run in MSYS2: pacman -S mingw-w64-i686-gcc"
        exit 1
    }
    
    if (-not (Test-Path $GCC)) {
        Write-Error "GCC (32-bit) not found at $GCC"
        Write-Host "Run in MSYS2: pacman -S mingw-w64-i686-gcc"
        exit 1
    }
...
function Invoke-Compile {
    ...
    Write-Step "Compiling $Type sources (32-bit)..."
...
function Invoke-Link {
    ...
    Write-Step "Linking $Type DLL (32-bit)..."
...
function Copy-Dependencies {
    ...
    Write-Step "Copying dependencies for $Type (32-bit)..."
...
function Invoke-Build {
    ...
    Write-ColorOutput "║  Windows 32-bit Crypto Library Builder    ║" "Cyan"
