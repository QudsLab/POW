...
/* DISABLED: This main() function is not needed when compiling as a library
// ==== Main ===
...
#define Maj_macro(x, y, z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))

static void sha256_process(sha256_ctx *ctx, const uint8_t *block) {
  uint32_t W[64];
  uint32_t a, b, c, d, e, f, g, h;
  int i;
  ...
  for (i = 0; i < 64; i++) {
    ...
    uint32_t T2 = SIGMA0(a) + Maj_macro(a, b, c);
    h = g;
    g = f;
    f = e;
    e = d + T1;
...
void sha256_update(sha256_ctx *ctx, const uint8_t *data, size_t len) {
  size_t idx = (ctx->count / 8) % SHA256_BLOCK_SIZE;
  ctx->count += len * 8;
